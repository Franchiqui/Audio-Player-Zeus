...
(function() {
  ...
  // Función para aplicar data-component-id a elementos basándose en el CSS
  function ensureComponentIds() {
    ...
    Object.entries(iconProperties).forEach(function([componentId, props]) {
      ...
      // Si no existe, buscar elementos que coincidan con el patrón del componentId
      // El formato típico es: {class}-{tag}-nth-child-{index}-...
  ...
  // Función para buscar elemento por componentId
  function findElementByComponentId(componentId) {
    // Primero intentar buscar por data-component-id (más confiable)
    let element = document.querySelector('[data-component-id="' + componentId + '"]');
    if (element) return element;
    
    // Si no se encuentra, intentar asegurar que los IDs estén aplicados
    ensureComponentIds();
    
    // Intentar buscar nuevamente después de asegurar IDs
  ...
  function applyIcons() {
    // Evitar ejecutar múltiples veces simultáneamente
    if (isApplyingIcons) {
      return;
    }
    ...
    // Primero asegurar que los data-component-id estén aplicados
    ensureComponentIds();
    ...
    setTimeout(function() {
      ...
      // Remover iconos anteriores de forma más agresiva
      ...
      // También buscar SVGs directamente en el elemento
      ...
      if (appliedCount > 0 || notFoundCount > 0) {
        console.log("[zeus-icons] 📊 Summary - Applied:", appliedCount, "Skipped:", skippedCount, "Not found:", notFoundCount, "Total:", Object.keys(iconProperties).length);
      }
      if (notFoundCount > 0) {
        console.warn("[zeus-icons] ⚠️ Some icons could not be applied. Make sure the CSS file is loaded and elements have data-component-id attributes.");
      }
  ...
  // Ejecutar cuando el DOM esté listo y React haya hidratado
  function initIcons() {
    ...
    // Next.js marca cuando la hidratación está completa
    function waitForHydration() {
      // Esperar a que el DOM esté completamente cargado
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", function() {
          // Esperar un poco más para que React termine de hidratar
          setTimeout(applyIcons, 100);
          setTimeout(applyIcons, 500);
          setTimeout(applyIcons, 1000);
        });
      } else {
        // Si el DOM ya está cargado, esperar a que React hidrate
        setTimeout(applyIcons, 100);
        setTimeout(applyIcons, 500);
        setTimeout(applyIcons, 1000);
  ...
  // Ejecutar solo después de que la página esté completamente cargada (evitar hidratación)
  if (typeof window !== "undefined" && typeof document !== "undefined") {
    // Esperar a que Next.js termine de hidratar
    if (document.readyState === "complete") {
      // Si ya está completo, esperar un poco más para React
  ...
  // También escuchar eventos de navegación de Next.js
  if (typeof window !== "undefined" && typeof document !== "undefined") {
    ...
    // Para Next.js App Router - escuchar eventos de navegación
